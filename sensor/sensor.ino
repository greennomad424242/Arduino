/*
Sketch to read a MS5803-14BA pressure sensor, written from scratch. Will output data to the serial console.
Written by Walt Holm
Initial revision 10 Oct 2013
Rev 1 12 Oct 2013 -- Implements 2nd order temperature compensation */
#include <Wire.h>

const int DevAddress = 0x76; //7-bitI2CaddressoftheMS5803 // Here are the commands that can be sent to the 5803
...
unsigned int CalConstant[8]; //Matrixforholdingcalibrationconstants

float Temperature, Pressure, TempDifference, Offset, Sensitivity; 
float T2, Off2, Sens2; //Offsetsforsecond-ordertemperaturecomputation
byte ByteHigh, ByteMiddle, ByteLow; 
...
// Serial.println(AdcPressure);
   ...
   delay(10); 
   sendCommand(AdcRead); 
   Wire.requestFrom(DevAddress, 3); while(Wire.available())
{
ByteHigh = Wire.read(); ByteMiddle = Wire.read(); ByteLow = Wire.read();
}
AdcTemperature = ((long)ByteHigh << 16) + ((long)ByteMiddle << 8) + (long)Byte // Serial.print("D2 is: ");
...
Temperature=Temperature+2000; //Thisisthetemperatureinhundredthsof
// Calculate the second-order offsets
if (Temperature<2000.0) //Istemperaturebeloworabove20.00degC?  
{
...
Temperature=Temperature/100; //ConverttodegreesC 
...
Pressure = (float)AdcPressure * Sensitivity / pow(2, 21); Pressure = Pressure - Offset;
Pressure = Pressure / pow(2, 15); Pressure=Pressure/10000; //Setoutputtobars;
// Convert to psig and display
Pressure=Pressure-1.015; //Converttogaugepressure(subtractatmospher Pressure=Pressure*14.50377; //Convertbarstopsi Serial.print("Pressure in psi is: ");
Serial.println(Pressure);
Serial.println();
delay(1000); //pausefor1secondbeforelooping 
